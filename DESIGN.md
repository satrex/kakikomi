...
## 15. ツールバーのアイコン化 v2.0.1

2026-08-04 のフィードバックで確定。

### 問題

ツールが3種→8種に増えた結果、ツールバーの必要幅がウィンドウ幅を超え、
右端の「ピクチャに保存」ボタンが見切れる。各ボタンが `Label(テキスト, systemImage:)`
でテキスト+アイコンを描画し、ツールピッカーも8セグメント全てにテキストがあるため。

### 対策: アイコンのみ表示（プレビュー.app のマークアップ風）

- ツールバーの全ボタンとツールピッカーの全セグメントを **SF Symbols のアイコンのみ**に
  する。**絵文字（Unicode 文字）は使わない**: 常にカラー描画のためボタンの
  無効状態やテーマに追従せず、macOS のバージョンで字形が変わるため
- 情報を失わないための必須条件（アイコン化とセットで守る）:
  - すべてのアイコンのみコントロールに `.help(...)` でツールチップを付ける
  - すべてに `.accessibilityLabel(...)` を付ける（VoiceOver 用。既存の
    `accessibilityIdentifier` はテスト用の別物なので両方必要）
- 簡易・詳細どちらのモードでもアイコンのみ（詳細モードの違いは
  サイドインスペクタの有無であって、ツールバーの粒度ではない）
